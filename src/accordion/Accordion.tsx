...
export default function Accordion({ state, size, data, focus, hover }: AccordionProps) {
        ...
        <FlexBox direction="col" className="w-full justify-between">
            {data.map((item, index) => (
                <div key={index} className="w-[400px] bg-layer-transparent">
                    ...
                    >
                        <span className={`label-02-regular m-spacing-04 ${stateStyle.inText[state]}`}>
                            {item.title}
                        </span>
                        <Icon
                            icon={isOpen[index] ? 'chevron_up_outlined' : 'chevron_down_outlined'}
                            size={24}
