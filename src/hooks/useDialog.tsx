...
function DialogTemplate({ title, discription, type, onAction, onClose }: DialogProps) {
    return (
        <FlexBox className="fixed inset-0 z-[100] justify-center bg-Dialog">
            <div className="w-[242px] mx-auto rounded-lg overflow-y-auto bg-white">
                ...
                <FlexBox direction="col" className="gap-4 py-3 border-t border-Gray2">
                    {type === 'notice' ? (
                        <button type="button" className="w-full B4-regular text-Gray5" onClick={onClose}>
                            확인
                        </button>
                    ) : (
                        <>
                            <button type="button" className="w-full B4-regular text-Red" onClick={onAction}>
                                삭제
                            </button>
                            <button type="button" className="w-full B4-regular text-Gray5" onClick={onClose}>
                                취소
...
function useDialog(): UseDialogType {
    ...
    const openDialog = (props: DialogProps) => {
        ...
        setDialog(<DialogTemplate {...props} onAction={onClickAction} onClose={onClickClose} />);
    };
